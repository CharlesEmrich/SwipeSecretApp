...
function getPopSecret() {
  ...
  return function popSecret(userId) {
    ...
    Promise.all([unwantedPromise, approversPromise])
    .then(([unwantedIds, approverIds]) => {
      unwantedIds.push(userId);
    ...
    .then(approvingUser => {
      if (approvingUser) return approvingUser;
        ...
        $nin: unwantedIds //not defined here. use Promise?
      }});
    })
...
// getPopSecret()('57aab8f8f0622b6832482bb8');
